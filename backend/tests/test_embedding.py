...
class CompatibleEmbeddingTests(unittest.TestCase):
    ...
    def test_signature_requires_a_returned_dimension(self) -> None:
        ...
        with self.assertRaisesRegex(EmbeddingDimensionUnknown, "维度尚未"):
            LangChainEmbeddingService(provider=UnknownDimensionProvider()).signature()
    ...
    def test_rejects_empty_input(self) -> None:
        ...
        service = LangChainEmbeddingService(provider=Provider())

        with self.assertRaisesRegex(ValueError, "不能为空"):
            service.embed_query("  ")


class LangChainEmbeddingFacadeTests(unittest.TestCase):
    def test_default_service_builds_langchain_embeddings_lazily(self) -> None:
        ...
        with patch(
            "app.rag.embeddings.build_embeddings",
            return_value=FakeEmbeddings(),
        ) as factory:
            service = LangChainEmbeddingService(config)
...
def test_cached_bge_embeddings_round_trip_through_faiss(tmp_path: Path) -> None:
    ...
    embedding = LangChainEmbeddingService(config)
    texts = [
