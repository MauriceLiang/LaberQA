# 劳动权益咨询问答台——系统设计与项目开发文档v2

> 文档版本：2.0（全部选做功能纳入正式范围）  
> 项目阶段：项目设计  
> 适用范围：劳动权益咨询问答台课程项目  
> 设计依据：项目任务书与需求文档
...
- `EvaluationDashboard.vue`：50+ 用例评测结果与指标；
...
最终评测规模为 **50 条以上**，建议首版 60 条（40 条库内 + 20 条库外）。评测数据由 `evaluation_case`、`evaluation_run` 与 `evaluation_result` 三部分组成。
...
新增 `AnswerStyleService` / Prompt 模板变量，支持：
...
新增 `RetrievalExperimentService`，统一管理实验矩阵和运行结果。每个实验配置至少包含：
...
实验固定使用同一批评测用例，输出正确率、拒答率、引用命中率、平均检索耗时，并根据主指标生成推荐配置。实验索引建议使用独立目录，避免不同 Chunk 配置相互覆盖。
...
| GET | `/api/retrieval-experiments/{id}` | 获取实验结果 | id | 对比表、指标、推荐配置 |
...
在全部选做纳入后，资料管理前端同时负责“待补充资料”页面；智能问答前端同时负责应答风格切换、评测仪表盘与检索实验页面。两名前端成员均需要提交实际业务代码。
...
12. 拒答问题是否进入高频无依据统计；
13. 待补充资料状态是否可维护；
14. 50 条以上评测是否可按批次执行与保存；
15. Chunk、Top-k、Rerank 实验是否能得到可比较结果。

### 9.2 正式评测集设计

建议使用 60 条固定测试用例作为课程最终评测集：
...
如时间允许，可再增加组合实验。最终以引用命中率和回答正确率为主，兼顾拒答率与耗时，给出推荐参数。
...
最终成果应保证：按照 README 在新的环境中能够完成依赖安装、配置、启动前后端，并完整演示“资料导入 → 知识库检索 → 问答 → 来源回查 → 多轮追问 → 无依据拒答 → Tool 展示 → 双应答风格 → 50+ 测试评测 → 高频缺失资料归纳 → 检索策略实验”的完整闭环。

---

## 12. 任务书覆盖检查结论

经逐项对照，任务书中的必做功能、技术要求与四项选做功能均已在本设计中落到具体模块、接口、数据结构、测试和验收流程。特别修正了旧版文档中“选做仍作为可选”“最终评测仍以 15 条为主”“缺少实验与待补资料的持久化设计”等问题。开发阶段应以本 2.0 文档为基线，最终代码和演示结果需与文档保持一致。
